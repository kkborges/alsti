...
class GeminiClient:
    ...
    def _build_prompt(
        ...
        parts.append(
            "\n### Instructions:\n"
            "- Return ONLY the code for THIS SPECIFIC FILE, nothing else\n"
            "- Do not include ```language``` code blocks or markdown formatting\n"
            "- Do not include explanations, comments about what you're doing, or example usage\n"
            "- Include only the necessary imports and code for THIS specific file\n"
            "- DO NOT include code that belongs in other files\n"
            "- DO NOT create multiple files or sections in the response\n"
            "- Add brief inline comments for complex logic only\n"
            "- Ensure the code is complete and can be executed/compiled immediately\n"
            "\n### CRITICAL - File-Specific Content:\n"
            "- If asked for 'config.py', include ONLY configuration code (no models, no routes)\n"
            "- If asked for 'models.py', include ONLY database models (no API logic, no main code)\n"
            "- If asked for 'main.py' or 'app.py', include ONLY the application entry point\n"
            "- If asked for 'routes.py' or API endpoints, include ONLY route handlers\n"
            "- Never mix concerns: keep database, business logic, routes, and config separate\n"
